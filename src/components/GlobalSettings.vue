...
const THEMES = [
  { id: 'light', label: 'Light', icon: 'fa-sun',   hint: 'The daytime theme' },
  { id: 'night', label: 'Night', icon: 'fa-mug-hot', hint: 'Warm paper after Solarized Light, on a dimmer ground than the day theme — for working in a low-lit room' },
  { id: 'dark',  label: 'Dark',  icon: 'fa-moon',  hint: 'Dark surfaces with light text' },
]
...
<template>
  <div class="card">
    <div class="gs-head">
      ...
      <div class="gs-seg" role="group" aria-label="Theme">
        <button v-for="t in THEMES" :key="t.id" type="button" class="gs-seg-btn"
                :class="{ active: store.theme === t.id }" :title="t.hint"
...
/* Theme + radius segmented controls */
...
.gs-seg-btn.active { background: var(--acc-fill); color: #fff; box-shadow: 0 2px 6px var(--acsh); }
...
.gs-toggle input:checked + .gs-switch { background: var(--acc-fill); }
