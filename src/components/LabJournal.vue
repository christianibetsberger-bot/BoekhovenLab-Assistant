...
onMounted(async () => {
    ...
    if (user) {
        ...
        // Supabase is the authoritative source for journal entries — they are
        // auto-saved there after 1 s of inactivity (saveToDb). Using a
        // localStorage draft to override cloud content broke cross-context
        // sync: the browser's draft would hide changes made in the PWA (and
        // vice versa) because each context has its own isolated localStorage.
